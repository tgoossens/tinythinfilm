function [T,Phi_t,Phi_in] = transmittanceTiny2DIntegral(filter,incident_wavepacket,angle_deg,wavelengths,polarization,accuracy,pixel)
...
%  http://github.com/tgoossens
    ...
    if(or(polarization=='unpolarized',polarization=='unpolarised'))
        [T_s,Phi_t_s,Phi_in_s] = transmittanceTiny2DIntegral(filter,incident_wavepacket,angle_deg,wavelengths,'s',accuracy,pixel);
        [T_p,Phi_t_p,Phi_in_p] = transmittanceTiny2DIntegral(filter,incident_wavepacket,angle_deg,wavelengths,'p',accuracy,pixel);
        T =  0.5*(T_s+T_p);
...
%    Phi_t(j)=trapz(nu,temp); 
    ...
    temp= 0.5*eta_sub(:,1,j).*At(:,1,j).*conj(At(:,1,j)).' .*width.*sinc((nu-nu')*width);
    Phi_t(j)=trapz(nu,trapz(nu',real(temp),1),2); 
