...
function [T,Phi_t,Phi_in] = tinytransmittance3dfocus_monothick(central_wavelength,normalized_fwhm,effective_index,width,nsub,coneangle_deg,wavelengths,polarization,pixelkernel,accuracy);
...
for j=1:numel(wl)
    ...
    % Incident wwave
    width_y=width;
    nucone = abs(sind(coneangle_deg))/wl(j);
    Ain(:,:,j) = circ(nu,nucone);
